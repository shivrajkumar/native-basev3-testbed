...
import { boolean, color, number, select, text, withKnobs, number } from "@storybook/addon-knobs";
...
storiesOf("Primitives", module)
  ...
  .add("Web Layout", () => (
    <Columns height={number("height", "100%")} flex={number("flex", 1)}>
      <Column bg="gray.2" flexGrow={number("flexGrow", 1)} width={0.15 / 1} />
      <Column flexGrow={number("flexGrow", 1)} bg="gray.3" width={0.85 / 1}>
        <Stack space={number("space", 3)} height={number("height", 200)} mb={number("mb", 3)}>
          <Box
